-- =============================================================================
-- FriendGraph — FIX: "table public.locations not found" / "nodes.location_id does not exist"
-- Run this entire file in Supabase → SQL Editor → Run
-- Safe to re-run: uses IF NOT EXISTS / DROP POLICY IF EXISTS where needed
-- After success, wait ~1 min or open API Settings → Restart PostgREST if cache is stale
-- =============================================================================

create extension if not exists "pgcrypto";

-- Relationship enum (used by nodes.relationship)
do $$
begin
...
-- New columns on nodes (additive — keeps legacy "attributes" until you drop it)
...
-- Backfill defaults where columns were just added
update public.nodes
set
...
update public.nodes
set relationship = 'friend'::public.node_relationship
where relationship is null;
...
create index if not exists nodes_location_id_idx on public.nodes (location_id);

-- Optional: migrate from legacy jsonb "attributes" when that column still exists
do $$
declare
...
-- Everyone with a node gets a "General" location if still unassigned
insert into public.locations (user_id, name)
...
-- Edges: non-null label (app expects this)
...
-- Optional: remove legacy column after you verify the app (uncomment next line)
-- alter table public.nodes drop column if exists attributes;

-- Hint PostgREST / API to refresh schema cache (Supabase)
